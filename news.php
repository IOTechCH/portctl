					...
					<div class="row">
						<?php
							if(isset($_GET['id']))
							{
								...
								echo "<br><a href='news.php' class='btn btn-primary'>Back to news article list...</a>";
							}
							else
							{
								$querydbq = $db->prepare('SELECT * FROM `news` ORDER BY `id` DESC LIMIT 20');
								$querydbq->execute(array());
								foreach($querydbq as $row)
								{
									$row['news_content'] = str_replace("<b>", "", $row['news_content']); // remove HTML formatting
									echo '<a href="news.php?id='.$row['id'].'"><b>'.$row['news_title'].'</b><small> posted by '.$row['news_author'].'</small><br>';
									echo substr($row['news_content'],0,38); echo ".... <br><button class='btn btn-primary'>Read More</button>";
									echo '</a>&nbsp;<br><br>';
								}
							}
						?>
