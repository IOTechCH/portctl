					...
					<div class="row">
						<?php
							if(isset($_GET['id']))
							{
								...
								echo "<br><a href='blog.php' class='btn btn-primary'>Back to blog article list...</a>";
							}
							else
							{
								$querydbq = $db->prepare('SELECT * FROM `blog` LIMIT 20');
								$querydbq->execute(array());
								if($querydbq->rowCount() == 0)
								{
									echo "No blog posts found.";
								}
								foreach($querydbq as $row)
								{
									echo '<a href="blog.php?id='.$row['id'].'"><b>'.$row['blog_title'].'</b><small> posted by '.$row['blog_author'].'</small><br>';
									echo substr($row['blog_content'],0,38); echo ".... <br><button class='btn btn-primary'>Read More</button>";
									echo '</a>&nbsp;<br>';
								}
							}
						?>
