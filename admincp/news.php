									...
									else
									{
										...
										if($do=="add")
										{
											...
											if(isset($_POST['submit']))
											{
												...
												$content = $_POST['news_article_content']; // allows HTML in article
												$sql = "INSERT INTO `news`(`id`, `news_title`, `news_content`, `news_author`) VALUES (NULL, ?, ?, ?)";
												$query = $db->prepare($sql);
												$query->execute(array($title, $content, $_SESSION['user']));
												echo "Article published. ";
												// Get article ID to give user the link
												$qone = $db->prepare("SELECT * FROM `news` WHERE `news_title` = ? AND `news_content` = ? LIMIT 1");
												$qone->execute(array($title, $content));
												foreach($qone as $row)
												{
													echo "<a href='../news.php?id=".$row['id']."' class='btn btn-primary'>View Article</a>";
												}
											}
											else
											{
												// Load form
												?>
													<form method="POST" action="news.php?do=add">
														<!-- Add Form -->
														News Article Title: <input type="text" name="title"><br>
														News Article Content:<br>
														<textarea name="news_article_content" style="width:100%;"></textarea>
