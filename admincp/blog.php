									...
									else
									{
										...
										if($do=="add")
										{
											...
											if(isset($_POST['submit']))
											{
												...
												$content = $_POST['blog_article_content']; // allows HTML in article
												$sql = "INSERT INTO `blog`(`id`, `blog_title`, `blog_content`, `blog_author`) VALUES (NULL, ?, ?, ?)";
												$query = $db->prepare($sql);
												$query->execute(array($title, $content, $_SESSION['user']));
												echo "Article published. ";
												// Get article ID to give user the link
												$qone = $db->prepare("SELECT * FROM `blog` WHERE `blog_title` = ? AND `blog_content` = ? LIMIT 1");
												$qone->execute(array($title, $content));
												foreach($qone as $row)
												{
													echo "<a href='../blog.php?id=".$row['id']."' class='btn btn-primary'>View Article</a>";
												}
											}
											else
											{
												// Load form
												?>
													<form method="POST" action="blog.php?do=add">
														<!-- Add Form -->
														Blog Article Title: <input type="text" name="title"><br>
														Blog Article Content:<br>
														<textarea name="blog_article_content" style="width:100%;"></textarea>
														<br><input type="submit" name="submit" value="Publish Article" class="btn btn-primary">
													</form>
												<?php
											}
										}
										else
										{
											if($do=="list")
											{
												echo '<a class="btn btn-primary" href="../blog.php">Click here to be redirected.</a>';
											}
										}
									}
								?>
