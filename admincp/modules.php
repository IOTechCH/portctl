...
<html class="fixed">
	<head>
		...
		<title>Modules | <?php echo $sitename; ?></title>
							...
							<section class="panel">
								<?php 
									if(isset($_GET['action']))
									{
										...
										{
											// DISABLE MODULE
											$mod_name = $_GET['module_name'];
											if(!isset($mod_name)) {
												echo "Module name not defined.";
											}
											else
											{
												$query = $db->prepare('UPDATE `pagehooks` SET `status`="0" WHERE `module` = ?');
												$query->execute(array($mod_name));
										...
										else
										{
											...
											{
												$query = $db->prepare('UPDATE `pagehooks` SET `status`="1" WHERE `module` = ?');
												$query->execute(array($mod_name));
									...
									{
										// List modules
										echo "Module Color Legend: BLUE = enabled, RED = disabled<br>";
										$query = $db->prepare('SELECT * FROM `pagehooks`');
										$query->execute();
										if($query->rowCount() == 0)
										{
										...
										else
										{
											...
											{
												// display modules
												echo "<a href='modules.php?action=";
												if($row['status'] == "0") {
													echo "enable&module_name=".$row['module'];
												}
												else
												{
													echo "disable&module_name=".$row['module'];
												}
												echo "' class='btn btn-";
												if($row['status'] == "0") {
													echo "danger";
												...
												echo "'>".$row['module']." (interface:".$row['interface']." position:".$row['position'].")</a><br><br>";
											}
										}
									}
								?>
