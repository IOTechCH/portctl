...
<html class="fixed">
	<head>
		...
		<title>Customer Search | <?php echo $sitename; ?></title>
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					...
					<div class="row">
								...
								<?php 
									if(isset($_POST['submit']))
									{
										...
										if($opt == "1")
										{
											...
											{
												echo "Order Information Located:<br>";
												foreach($query as $row)
												{
													echo "Order Email: ".$row['order_email']."<br>Order Status: ".$row['order_status']."<br>Order PID: ".$row['order_pid']."<br>";
												}
												echo "Order Status Legend: 0 is not-approved, 1 is approved, 2 is paid, 3 is cancelled.";
											}
										}
										else if($opt == "2")
										{
											// SEARCH VIA CUSTOMER ID
											$customer_id = $_POST['customer_id'];
											if(!is_numeric($customer_id)) {exit('Customer ID must not contain any special chars, or words.');}
											if(!isset($customer_id)) {exit('Error: customer id not defined.');}
											$query = $db->prepare('SELECT * FROM `customers` WHERE `customer_id` = ?');
											$query->execute(array($customer_id));
											if($query->rowCount() == 0)
											{
												echo "No customers for that customer id were located.";
											}
											else
											{
												echo "Customer Information Located:<br>";
												foreach($query as $row)
												{
													echo "Customer ID: ".$row['customer_id']."<br>";
													echo "Customer Username: ".$row['customer_user']."<br>";
													echo "Customer Password (HASHED): ".$row['customer_pass']."<br>";
												}
											}
										}
									}
						...
						<div class="col-md-6 col-lg-12 col-xl-6">
							<section class="panel">
								<!-- Pending Customer Search List -->
								Search By Email:
								<form method="POST" action="customer_search.php?opt=1">
								...
								<form method="POST" action="customer_search.php?opt=2">
									<input type="text" name="customer_id" value="" placeholder="2502590259201"><input type="submit" name="submit" value="Search" class="btn btn-primary">
								</form>
