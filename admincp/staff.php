...
<html class="fixed">
	<head>
		...
		<title>Staff Management | <?php echo $sitename; ?></title>
						...
						<div class="col-md-6 col-lg-12 col-xl-6">
							<section class="panel">
								<?php 
									...
									{
										$do = $_GET['do'];
										if($do=="list" or $do=="view")
										{
											// load html menu
											?>
											<a href="staff.php?do=edit" class="btn btn-primary">Search/Modify For Staff</a> <a href="staff.php?do=add" class="btn btn-primary">Add Staff</a>
											<?php
										}
										else if($do=="edit")
										{
											...
											{
												// Look in DB for staff member
												if(!is_numeric($_GET['staff_id']))
												{
													exit("Staff id not defined.");
												}
												$query = $db->prepare('SELECT * FROM `admin_users` WHERE `id` = ?');
												$query->execute(array($_GET['staff_id']));
												foreach($query as $row)
												{
													$staff_user = $row['username'];
												}
												if(isset($_POST['submit']))
												{
													...
													if(empty($_POST['password']))
													{
														...
														$user = $_POST['username'];
														$acl = $_POST['acl'];
														if(!is_numeric($acl)) {exit("ACL must be 0 or 1");}
														$query = $db->prepare('UPDATE `admin_users` SET `username` = ? AND `access_level` = ? WHERE `id` = ?');
														$query->execute(array($user, $acl, $_GET['staff_id']));
													...
													else
													{
														...
														$acl = $_POST['acl'];
														if(!is_numeric($acl)) {exit("ACL must be 0 or 1");}
														$query = $db->prepare('UPDATE `admin_users` SET `username` = ? AND `password` = ? AND `access_level` = ? WHERE `id` = ?');
														$query->execute(array($user, $pass, $acl, $_GET['staff_id']));
														echo "Updated user successfully!";
													}
												}
												else
												{
													?>
														<form method="POST">
															Username: <input type="text" name="username" value="<?php echo $staff_user; ?>"><br>
															Password: <input type="password" name="password"><br>
															ACL: <input type="text" name="acl" value="0"><br>
															<input type="submit" name="submit" value="Update Staff" class="btn btn-primary">
														</form>
													<?php
												}
											}
											else
											{
												...
												$query = $db->prepare('SELECT * FROM `admin_users` WHERE `access_level` = "1"');// change to 0, 1 is just for testing
												$query->execute(array());
												foreach($query as $row)
												{
													echo '<a href="staff.php?do=edit&staff_id='.$row['id'].'" class="btn btn-primary">Edit '.$row['username'].'</a>';
												}
											}
										}
										else if($do=="add")
										{
											...
											if(isset($_POST['submit']))
											{
												if($_POST['csrf_token']!=$_SERVER['REMOTE_ADDR'])
												{
													exit("CSRF mismatch.");
												}
												// Save to database
												$query = $db->prepare('INSERT INTO `admin_users`(`id`, `username`, `password`, `access_level`) VALUES (NULL, ?, ?, ?)');
												$query->execute($_POST['username'], $_POST['password'], $_POST['acl']);
												echo "Staff member was added successfully.<br>";
											}
											else
											{
												// Show adding form
												?>
													<form method="POST">
														...
														<input type="submit" name="submit" value="Add Staff Member" class="btn btn-primary">
													</form>
												<?php
											}
										}
